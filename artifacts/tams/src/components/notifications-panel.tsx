...
export function NotificationBell() {
  ...
  const { data: notifications = [], isLoading } = useGetNotifications({
    query: { queryKey: ['notifications'], refetchInterval: 60_000, staleTime: 30_000 } as any,
  });

  const criticalCount = notifications.filter((n: Notification) => n.severity === "critical").length;
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-10 w-80 max-h-[480px] z-50 bg-card border border-card-border rounded-xl shadow-xl flex flex-col animate-fade-in">
            ...
            <div className="overflow-y-auto flex-1 py-1" aria-live="polite">
              {isLoading ? (
