...
const actions = [
  ...
  ["repo.validate", "Validation repo", "GitHub read-only"],
          ...
          return (
            <article key={id} className="rounded-2xl border border-border bg-card/70 p-4 space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div><h2 className="font-medium">{label}</h2><p className="text-xs text-muted-foreground">{id}</p></div>
                {result && <span className={`rounded-full border px-2 py-0.5 text-[11px] ${badge(result.status)}`}>{result.status}</span>}
              </div>
              <p className="text-sm text-muted-foreground">{description}</p>
              <button type="button" className="rounded-lg border border-primary/30 bg-primary/10 px-3 py-1.5 text-xs font-medium text-primary" onClick={() => setActive(isActive ? "" : id)}>{isActive ? "Fermer" : "Tester"}</button>
              {isActive && (
                <div className="space-y-3 rounded-xl border border-border bg-background/60 p-3">
                  <textarea rows={4} value={inputs[id] ?? defaultInput(id)} onChange={(event) => setInputs(prev => ({ ...prev, [id]: event.target.value }))} className="w-full rounded-lg border border-border bg-background p-3 text-sm outline-none focus:border-primary" />
                  <button type="button" disabled={running === id} onClick={() => run(id)} className="inline-flex items-center gap-2 rounded-lg bg-primary px-3 py-2 text-xs font-medium text-primary-foreground disabled:opacity-60">{running === id && <Loader2 className="h-3.5 w-3.5 animate-spin" />}Exécuter</button>
...
function ResultView({ result }: { result: RunResult }) {
  return (
    <div className="space-y-3 rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3 text-sm">
      <div className="flex flex-wrap items-center gap-2"><span className={`rounded-full border px-2 py-0.5 text-[11px] ${badge(result.status)}`}>{result.status}</span><span className={`rounded-full border px-2 py-0.5 text-[11px] ${badge(result.mode)}`}>{result.mode}</span>{result.providerUsed && <span className="text-xs text-muted-foreground">Provider : {result.providerUsed}</span>}</div>
      ...
      {result.artifact?.url && <a className="inline-flex rounded-lg border border-primary/30 bg-primary/10 px-3 py-2 text-xs font-medium text-primary underline" href={result.artifact.url} target="_blank" rel="noreferrer">Ouvrir le fichier</a>}
