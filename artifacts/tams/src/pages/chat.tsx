    ...
    <div className="absolute bottom-full left-0 right-0 mb-1 bg-card border border-border rounded-xl shadow-lg overflow-hidden z-50">
      {filtered.map((cmd, i) => (
        <button
          ...
          className={cn("w-full flex items-center gap-3 px-3 py-2.5 text-left transition-colors", i === selected ? "bg-accent" : "hover:bg-accent/50")}
        >
...
function TypingIndicator() {
  return (
    <div className="flex gap-1 items-center py-1">
      {[0, 1, 2].map(i => (
        <div key={i} className="w-2 h-2 rounded-full bg-muted-foreground/40 animate-wave" style={{ animationDelay: `${i * 150}ms` }} />
      ))}
    </div>
  );
}
...
/* ─── Action status bar ─── */
function ActionStatusBar({ mode, messageCount, isStreaming, isError, onRetry, onNewConversation }: { mode: Mode; messageCount: number; isStreaming: boolean; isError: boolean; onRetry: () => void; onNewConversation: () => void; }) {
    ...
    <div className="shrink-0 px-4 py-1.5 border-b border-border/50 bg-sidebar/50 backdrop-blur-sm">
      <div className="flex items-center gap-3">
        <div className={cn("flex items-center gap-1.5 text-[10px] font-medium px-2 py-0.5 rounded-full", modeColor[mode] ?? modeColor.chat)}>
          <ModeIcon className="w-3 h-3" /><span>{modeLabel}</span>
        </div>
        ...
        {isError ? (
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 text-[10px] text-destructive"><WifiOff className="w-3 h-3" /><span>Erreur de connexion</span></div>
            <button onClick={onRetry} className="flex items-center gap-1 text-[10px] text-primary hover:text-primary/80 transition-colors"><RotateCcw className="w-3 h-3" />Réessayer</button>
            <button onClick={onNewConversation} className="flex items-center gap-1 text-[10px] text-muted-foreground hover:text-foreground transition-colors"><Plus className="w-3 h-3" />Nouvelle</button>
          </div>
        ) : (
          <div className="flex items-center gap-1 text-[10px] text-muted-foreground"><Wifi className="w-3 h-3 text-emerald-400" /><span>Connecté</span></div>
        )}
        {isStreaming && <div className="w-24"><Progress value={undefined} className="h-1" /></div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="absolute right-0 top-0 bottom-0 w-80 bg-card border-l border-border z-20 flex flex-col animate-slide-in shadow-xl">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {memories.length === 0 ? (
          <div className="text-center py-8"><Brain className="w-8 h-8 text-muted-foreground/30 mx-auto mb-2" /><p className="text-xs text-muted-foreground">Aucun contexte mémoire actif</p></div>
        ) : (
          memories.map((mem) => (
            <div key={mem.id} className="p-3 rounded-xl bg-secondary/50 border border-border/50 hover:border-border transition-colors">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="flex items-center gap-1">
                  <div className="w-8 h-1 bg-secondary rounded-full overflow-hidden">
                    <div className="h-full bg-emerald-400 rounded-full transition-all" style={{ width: `${mem.relevance * 100}%` }} />
                  </div>
