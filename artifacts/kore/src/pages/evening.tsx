...
import { useSubmitEveningReview, getListEveningReviewsQueryKey, useListEveningReviews } from "@workspace/api-client-react";
...
export default function Evening() {
  ...
  const [koreResponseText, setKoreResponseText] = useState("");
  ...
  const hasReviewedToday = pastReviews && pastReviews.length > 0 && 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.koreResponse) {
        setKoreResponseText(res.koreResponse);
      ...
      <div className="min-h-screen flex items-center justify-center p-8">
        <div className="text-center space-y-6 max-w-md">
          ...
          <p className="text-muted-foreground text-lg">
            La journée est finie. Vous avez fait de votre mieux. Déconnectez.
          </p>
        </div>
      </div>
    );
        ...
        {!showResponse ? (
          ...
          >
            ...
            <div className="space-y-12">
              ...
              <div className="space-y-8 bg-card/30 p-8 rounded-xl border border-card-border">
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <label className="text-foreground font-medium block">
                    Ce qui est reporté (sans culpabilité)
                  </label>
                  <Textarea 
                    value={deferredItems}
              ...
              <div className="space-y-4">
                ...
                <Textarea 
                  ...
                  className="bg-card border-card-border min-h-[150px] resize-none"
                  placeholder="Videz votre esprit..."
                />
              </div>
              ...
              <div className="flex justify-center pt-8">
                ...
                >
                  {submitReview.isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                    <>
                      Clore la journée
          ...
          >
            ...
            <div className="text-center space-y-6 max-w-2xl">
              <h2 className="text-2xl font-serif text-foreground">KORE</h2>
              <div className="bg-card/50 border border-card-border p-8 rounded-2xl text-left">
                <p className="text-lg leading-relaxed text-foreground/90 whitespace-pre-wrap">
                  {koreResponseText}
                </p>
              </div>
              <p className="text-muted-foreground mt-8">Vous pouvez maintenant fermer l'application.</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
