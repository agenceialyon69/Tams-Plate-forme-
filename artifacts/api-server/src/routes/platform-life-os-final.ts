...
const PERMISSION_ACTIONS: PermissionAction[] = [
  { actionId: "email.send", label: "Envoyer un email", description: "Envoi externe via Gmail.", riskLevel: "high", requiredPermission: "approved", humanApprovalRequired: true, dryRunSupported: true, externalSideEffect: true, rollbackPlan: "Aucun rollback fiable aprÃ¨s envoi.", source: "gmail" },
  { actionId: "calendar.write", label: "Modifier le calendrier", description: "CrÃ©er ou modifier un Ã©vÃ©nement.", riskLevel: "high", requiredPermission: "approved", humanApprovalRequired: true, dryRunSupported: true, externalSideEffect: true, rollbackPlan: "Supprimer ou restaurer l'Ã©vÃ©nement depuis l'audit.", source: "calendar" },
  { actionId: "n8n.run", label: "Lancer un workflow n8n", description: "DÃ©clenche un workflow externe.", riskLevel: "high", requiredPermission: "approved", humanApprovalRequired: true, dryRunSupported: true, externalSideEffect: true, rollbackPlan: "DÃ©pend du workflow; dry-run obligatoire si disponible.", source: "automation" },
  { actionId: "recovery.import", label: "Importer une sauvegarde", description: "Import append-only de donnÃ©es restaurÃ©es.", riskLevel: "critical", requiredPermission: "admin_only", humanApprovalRequired: true, dryRunSupported: true, externalSideEffect: false, rollbackPlan: "Exporter avant import et supprimer manuellement les lignes importÃ©es si nÃ©cessaire.", source: "recovery" },
  { actionId: "github.write", label: "Modifier GitHub", description: "Ã‰criture de code, branche ou mÃ©tadonnÃ©es GitHub.", riskLevel: "critical", requiredPermission: "admin_only", humanApprovalRequired: true, dryRunSupported: true, externalSideEffect: true, rollbackPlan: "Revert ciblÃ© sur branche; jamais de mutation directe de main.", source: "dev-agent" },
  ...
  { actionId: "github.pr.create", label: "CrÃ©er une PR", description: "CrÃ©e une pull request depuis une branche sÃ»re.", riskLevel: "high", requiredPermission: "approved", humanApprovalRequired: true, dryRunSupported: true, externalSideEffect: true, rollbackPlan: "Fermer la PR sans merge.", source: "dev-agent-ci" },
  { actionId: "worker.long.run", label: "ExÃ©cuter un worker long", description: "DÃ©marre une tÃ¢che longue bornÃ©e.", riskLevel: "medium", requiredPermission: "approved", humanApprovalRequired: true, dryRunSupported: true, externalSideEffect: false, rollbackPlan: "Annuler le job.", source: "jobs" },
  { actionId: "data.bulk.write", label: "Modifier des donnÃ©es en masse", description: "Ã‰criture interne volumique.", riskLevel: "critical", requiredPermission: "admin_only", humanApprovalRequired: true, dryRunSupported: true, externalSideEffect: false, rollbackPlan: "Snapshot/export prÃ©alable puis restauration append-only contrÃ´lÃ©e.", source: "platform" },
  { actionId: "automation.enable", label: "Activer une automation", description: "Active une exÃ©cution rÃ©currente.", riskLevel: "medium", requiredPermission: "approved", humanApprovalRequired: true, dryRunSupported: true, externalSideEffect: false, rollbackPlan: "DÃ©sactiver l'automation.", source: "automation" },
  ...
  { actionId: "life.capture", label: "Capturer une information", description: "Ã‰criture interne limitÃ©e et traÃ§able.", riskLevel: "low", requiredPermission: "preview", humanApprovalRequired: false, dryRunSupported: true, externalSideEffect: false, rollbackPlan: "Archiver ou supprimer la capture identifiÃ©e.", source: "life-os" },
];
...
async function logLifeEvent(input: {
  ...
  try {
    ...
    // L'observabilitÃ© ne doit jamais faire tomber une action utilisateur.
  }
}
...
async function checkPermission(action: PermissionAction, current: PermissionLevel, dryRun: boolean): Promise<{ allowed: boolean; reason: string }> {
  if (dryRun && action.dryRunSupported) return { allowed: true, reason: "Dry-run sans effet externe autorisÃ©." };
  ...
  if (await activeApproval(action.actionId)) return { allowed: true, reason: "Approval humain actif trouvÃ©." };
  return { allowed: false, reason: `Action ${action.riskLevel} bloquÃ©e: approval ${action.requiredPermission} requis.` };
}
...
const PermissionDenyBody = z.object({
  actionId: z.string().min(2),
  deniedBy: z.string().min(2).max(120),
...
router.post("/permissions/approve", async (req, res) => {
  ...
  await logLifeEvent({ type: "permission_approved", category: "security", severity: "warning", summary: `Approval temporaire crÃ©Ã© pour ${action.actionId}.`, source: "permission-system", relatedEntityType: "permission_approval", relatedEntityId: id });
...
router.post("/permissions/deny", async (req, res) => {
  const parsed = PermissionDenyBody.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ ok: false, error: "invalid_input", details: parsed.error.issues });
  const action = PERMISSION_ACTIONS.find(item => item.actionId === parsed.data.actionId);
  if (!action) return res.status(404).json({ ok: false, error: "unknown_action" });
  await logLifeEvent({
    ...
    category: "security",
    severity: "warning",
    summary: `Approval refusé pour ${action.actionId}: ${parsed.data.reason}`,
    source: "permission-system",
    relatedEntityType: "permission_action",
    relatedEntityId: action.actionId,
    metadata: { deniedBy: parsed.data.deniedBy },
  });
  return res.json({ ok: true, denied: { actionId: action.actionId, deniedBy: parsed.data.deniedBy, reason: parsed.data.reason } });
});
...
async function saveJob(job: JobRecord): Promise<void> {
  ...
  try {
    ...
    // Repli mÃ©moire bornÃ©: utile si la DB est indisponible, sans effet externe.
...
async function createJob(type: string, input: Record<string, unknown>, timeoutSeconds = 120): Promise<JobRecord> {
  ...
  await saveJob(job);
  await appendJobLog(job.id, "info", "Job crÃ©Ã© et mis en file.");
...
async function runJob(id: string): Promise<void> {
  ...
  await saveJob(job); await appendJobLog(id, "info", "ExÃ©cution dÃ©marrÃ©e hors de la requÃªte HTTP.");
  try {
    ...
    if (job.type === "gmail_sync" && integrations.gmail.status !== "connected") {
      job.status = "warn"; job.progress = 100; job.result = { status: integrations.gmail.status, synced: 0 }; job.lastError = "Gmail OAuth non vÃ©rifiÃ©.";
    } else if (job.type === "calendar_sync" && integrations.calendar.status !== "connected") {
      job.status = "warn"; job.progress = 100; job.result = { status: integrations.calendar.status, synced: 0 }; job.lastError = "Calendar OAuth non vÃ©rifiÃ©.";
    } else if (["video.generate", "audio.music.generate", "recovery_import", "dev_agent_repair"].includes(job.type)) {
      job.status = "warn"; job.progress = 100; job.result = { status: "handoff_required", safe: true }; job.lastError = "Ce job exige un provider ou un approval dÃ©diÃ©; aucun effet externe exÃ©cutÃ©.";
    } else {
      job.status = "success"; job.progress = 100; job.result = { status: "success", processedAt: nowIso(), safe: true };
    }
    job.updatedAt = nowIso();
    await saveJob(job);
    await appendJobLog(id, job.status === "success" ? "info" : "warning", job.lastError ?? "Job terminÃ©.");
...
router.post("/jobs", async (req, res) => {
  ...
  if (!verdict.allowed) {
    await logLifeEvent({ type: "permission_blocked", category: "security", severity: "warning", summary: `Job ${parsed.data.type} bloquÃ©: ${verdict.reason}`, source: "job-runner" });
...
router.post("/jobs/:id/cancel", async (req, res) => {
  ...
  await saveJob(job); await appendJobLog(job.id, "warning", "Annulation demandÃ©e.");
...
router.get("/memory-graph/v2/context", async (req, res) => {
  ...
  return res.json({ ok: true, query, factsKnown: nodes, recentEvidence: events, assumptions: nodes.length === 0 ? ["Contexte mÃ©moire v2 insuffisant."] : [], sources: [...new Set([...nodes, ...events].map(item => item.source))] });
});
...
for (const kind of ["gmail", "calendar"] as const) {
  ...
  router.get(`/life-os/integrations/${kind}/${kind === "gmail" ? "important" : "events"}`, (_req, res) => {
    ...
    return res.json({ ok: true, integration: kind, ...state, items: [], honestNote: state.status === "connected" ? "Aucune synchronisation exÃ©cutÃ©e dans cette requÃªte." : "Connexion indisponible; aucune donnÃ©e simulÃ©e." });
  });
...
async function seedAutomations(): Promise<void> {
  for (const item of AUTOMATION_TEMPLATES) {
    await pool.query(`INSERT INTO life_automations (id,title,description,enabled,schedule,type,required_permission,dry_run,approval_required)
      VALUES ($1,$2,$3,false,$4,$4,$5,true,$6) ON CONFLICT (id) DO NOTHING`, [item[0], item[1], `${item[1]} â€” moteur interne safe/local.`, item[2], item[3], item[4]]).catch(() => {});
  }
}
...
router.post("/life-os/automations/:id/run", async (req, res) => {
  ...
  await pool.query(`INSERT INTO life_automation_logs (automation_id,status,message,job_id,metadata) VALUES ($1,'queued',$2,$3,$4::jsonb)`, [automation.id, dryRun ? "Dry-run planifiÃ©." : "ExÃ©cution approuvÃ©e planifiÃ©e.", job.id, JSON.stringify({ dryRun })]).catch(() => {});
...
async function coachPayload(body: z.infer<typeof CoachBody>, redTeam = false) {
  ...
  const healthSignal = /douleur|santÃ©|fatigue|stress|sommeil/i.test(body.prompt);
  const familySignal = /famille|couple|enfant|fille|proche/i.test(body.prompt);
  const adminSignal = /facture|amende|dette|impÃ´t|papier|Ã©chÃ©ance|juridique/i.test(body.prompt);
  ...
  const sensitive = healthSignal || /juridique|dette|investir|mÃ©dic/i.test(body.prompt);
  const recommendation = energy < 35
    ? "RÃ©duire l'ambition: une seule action rÃ©versible de 10 minutes, puis repos ou aide humaine."
    : priorityDomain === "projects"
      ? "VÃ©rifier d'abord santÃ©, famille, admin et stabilitÃ©; ensuite choisir une micro-action projet de 30 minutes maximum."
      : `Traiter d'abord le domaine ${priorityDomain} avec une action courte, datÃ©e et vÃ©rifiable.`;
  ...
  const payload = {
    ...
    diagnosis: `PrioritÃ© dÃ©tectÃ©e: ${priorityDomain}. Ã‰nergie dÃ©clarÃ©e: ${energy}/100.`,
    factsKnown: [
      `Ã‰nergie fournie: ${energy}/100.`,
      `${events.length} Ã©vÃ©nement(s) Life OS rÃ©cent(s) disponible(s).`,
      `${jobs.filter(job => job.status === "fail" || job.status === "warn").length} job(s) rÃ©cent(s) en Ã©chec ou avertissement.`,
      `Gmail: ${integrations.gmail.status}; Calendar: ${integrations.calendar.status}.`,
    ],
    assumptions: events.length === 0 ? ["Le contexte historique est incomplet; la recommandation reste prudente."] : ["La demande reflÃ¨te la prioritÃ© immÃ©diate de l'utilisateur."],
    risks: [energy < 35 ? "Surcharge et mauvaise dÃ©cision sous faible Ã©nergie." : "Dispersion si plusieurs fronts sont ouverts.", sensitive ? "Sujet sensible: un professionnel qualifiÃ© peut Ãªtre nÃ©cessaire." : "Aucun risque clinique/juridique/financier n'est diagnostiquÃ©."],
    tradeoffs: ["Vitesse contre sÃ©curitÃ©.", "Ambition projet contre santÃ©, famille et stabilitÃ©."],
    recommendation,
    redTeamCounterArguments: ["Quelle preuve contredit cette prioritÃ© ?", "Quel coÃ»t cachÃ© en sommeil, argent ou temps familial ?", "Peut-on rÃ©duire l'action de moitiÃ© ?"],
    nextAction: priorityDomain === "health" ? "Noter les symptÃ´mes factuels et contacter un professionnel si douleur, danger ou persistance." : recommendation,
    timebox: energy < 35 ? 10 : 25,
    confidence: events.length > 0 ? 0.78 : 0.58,
    sources,
    limits: ["Pas de diagnostic mÃ©dical.", "Pas de conseil juridique dÃ©finitif.", "Pas de promesse financiÃ¨re.", integrations.gmail.status !== "connected" ? "Gmail non connectÃ©." : null, integrations.calendar.status !== "connected" ? "Calendar non connectÃ©." : null].filter(Boolean),
    doctrine: "SantÃ© â†’ famille â†’ admin/finance â†’ stabilitÃ© professionnelle â†’ projets â†’ apprentissage.",
  };
...
router.get("/life-os/coach/briefing", async (_req, res) => res.json(await coachPayload({ prompt: "PrÃ©pare mon briefing quotidien prudent.", energy: 50 })));
