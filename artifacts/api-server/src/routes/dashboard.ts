...
const DASHBOARD_TTL_S = 5 * 60; // 5 minutes in seconds
const DASHBOARD_CACHE_KEY = "dashboard:summary";

async function getCachedDashboard(): Promise<unknown | null> {
  return dbGet(DASHBOARD_CACHE_KEY);
}

async function setCachedDashboard(data: unknown): Promise<void> {
  await dbSet(DASHBOARD_CACHE_KEY, data, DASHBOARD_TTL_S);
}
...
router.get("/dashboard/summary", async (req, res) => {
  try {
    ...
    await setCachedDashboard(result);
