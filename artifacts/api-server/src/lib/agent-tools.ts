...
export async function gatherUserContext(): Promise<string> {
  try {
    ...
    return [
      tasks.length > 0 ? `Tâches actives (${tasks.length}): ${tasks.map((t: any) => `"${t.title}" (${t.priority}${t.dueDate ? `, due ${t.dueDate}` : ""})`).join(", ")}` : "",
      projects.length > 0 ? `Projets actifs: ${projects.map((p: any) => `"${p.name}"`).join(", ")}` : "",
      contacts.length > 0 ? `Contacts récents: ${contacts.map((c: any) => `${c.name}${c.company ? ` (${c.company})` : ""}`).join(", ")}` : "",
      memories.length > 0 ? `Mémoires récentes: ${memories.map((m: any) => `"${m.title}" [${m.type}]`).join(", ")}` : "",
      decisions.length > 0 ? `Décisions en cours: ${decisions.map((d: any) => `"${d.title}"`).join(", ")}` : "",
    ].filter(Boolean).join("\n");
  } catch {
