...
async function createLocalFallbackFrame(dest: string, index: number): Promise<void> {
  const colors = ["0x111827", "0x312e81", "0x064e3b"];
  await spawnFfmpeg(
    ["-y", "-f", "lavfi", "-i", `color=c=${colors[index % colors.length]}:s=${W}x${H}:d=1`, "-frames:v", "1", dest],
    30_000,
  );
}
...
export interface VideoResult {
  ...
  withText: boolean;
  withMusic: boolean;
  source: "provided_images" | "local_fallback";
}
...
export async function generateSlideshowVideo(opts: {
  ...
  try {
    const scaled: string[] = [];
    let fallbackUsed = urls.length === 0;
    ...
    if (scaled.length === 0) {
      fallbackUsed = true;
      for (let i = 0; i < 3; i++) {
        const fallback = path.join(work, `fallback${i}.jpg`);
        await createLocalFallbackFrame(fallback, i);
        scaled.push(fallback);
      }
    }
    ...
    return {
      ...
      withText: !!textFile,
      withMusic: !!musicPath,
      source: fallbackUsed ? "local_fallback" : "provided_images",
    };
  } finally {
