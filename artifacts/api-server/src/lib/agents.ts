...
// ─── Video Generation Intent Detection (PR #107) ────────────────────────────────

const VIDEO_GENERATION_KEYWORDS = [
  "génère une vidéo", "genere une video", "génère un vidéo", "genere un video",
  "génère la vidéo", "genere la video", "génère le vidéo", "genere le video",
  "crée une vidéo", "cree une video", "crée un vidéo", "cree un video",
  "créer une vidéo", "creer une video", "créer un vidéo", "creer un video",
  "fais une vidéo", "fais un video", "fait une vidéo", "fait un video",
  "vidéo tiktok naturel", "video tiktok naturel",
  "vidéo tiktok naturelle", "video tiktok naturelle",
  "tiktok naturel premium", "tiktok naturelle premium",
  "ugc naturel", "ugc naturelle",
  "vidéo réaliste", "video realiste", "vidéo realiste",
  "vraie vidéo", "vraie video", "vrai vidéo", "vrai video",
  "pub tiktok haute conversion", "publicité tiktok",
  "vidéo premium", "video premium",
  "impossible de savoir que c'est ia", "indiscernable de l'ia",
];

const SCRIPT_STORYBOARD_KEYWORDS = [
  "prépare un storyboard", "prepare un storyboard", "prépare un script", "prepare un script",
  "fais un storyboard", "fait un storyboard", "crée un storyboard", "cree un storyboard",
  "écris un script", "ecris un script", "rédige un script", "redige un script",
  "donne un prompt kling", "donne un prompt runway", "donne un prompt veo",
  "prépare un prompt kling", "prepare un prompt runway", "prepare un prompt veo",
  "storyboard pour", "script pour", "prompt pour",
];
...
function isVideoGenerationIntent(message: string): boolean {
  const normalized = normalizeText(message);
  // Si c'est une demande de script/storyboard, ce n'est PAS une génération vidéo
  if (SCRIPT_STORYBOARD_KEYWORDS.some(k => normalized.includes(normalizeText(k)))) {
    return false;
  }
  return VIDEO_GENERATION_KEYWORDS.some(k => normalized.includes(normalizeText(k)));
}

const VIDEO_HONESTY_RESPONSE = `Je ne peux pas générer une vraie vidéo IA réaliste depuis ce chat car aucun provider vidéo premium n'est connecté. Aucun fichier vidéo n'a été généré.

Je peux seulement :
1. Préparer un script/storyboard,
2. Préparer un prompt Kling/Runway/Veo,
3. Créer un prototype local FFmpeg avec vos images produit,
4. Ou utiliser un provider premium si vous le connectez.`;
...
export async function orchestrate(task: string): Promise<OrchestrationResult> {
  // PRIORITY: Check for video generation intent BEFORE any agent routing
  if (isVideoGenerationIntent(task)) {
    return {
      plan: { rationale: "Détection de demande de génération vidéo premium sans provider.", delegations: [] },
      results: [],
      synthesis: VIDEO_HONESTY_RESPONSE,
    };
  }
