...
const TABLES = [
  ...
  // Cache persistant (dashboard/graph/briefing…) utilisé par lib/cache-db.ts.
  // ABSENT ici = toute mutation invalidant le cache renvoyait 500 après avoir
  // pourtant écrit la ligne (tâches/décisions/mémoires « échouaient » à tort).
  `CREATE TABLE IF NOT EXISTS cache (key TEXT PRIMARY KEY, value JSONB NOT NULL, expires_at TIMESTAMP)`,
  `CREATE INDEX IF NOT EXISTS idx_cache_expires_at ON cache(expires_at)`,
  `CREATE TABLE IF NOT EXISTS life_events (id TEXT PRIMARY KEY, type TEXT NOT NULL, category TEXT NOT NULL, severity TEXT NOT NULL DEFAULT 'info', confidence DOUBLE PRECISION NOT NULL DEFAULT 1, summary TEXT NOT NULL, source TEXT NOT NULL, related_entity_type TEXT, related_entity_id TEXT, metadata JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS jobs (id TEXT PRIMARY KEY, type TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'queued', progress INTEGER NOT NULL DEFAULT 0, input JSONB NOT NULL DEFAULT '{}'::jsonb, result JSONB, last_error TEXT, timeout_at TIMESTAMP, cancel_requested BOOLEAN NOT NULL DEFAULT false, created_at TIMESTAMP NOT NULL DEFAULT now(), updated_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS job_logs (id SERIAL PRIMARY KEY, job_id TEXT NOT NULL REFERENCES jobs(id) ON DELETE CASCADE, level TEXT NOT NULL, message TEXT NOT NULL, metadata JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS permission_approvals (id TEXT PRIMARY KEY, action_id TEXT NOT NULL, status TEXT NOT NULL, approved_by TEXT NOT NULL, reason TEXT, expires_at TIMESTAMP, metadata JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS life_automations (id TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT NOT NULL, enabled BOOLEAN NOT NULL DEFAULT false, schedule TEXT NOT NULL, type TEXT NOT NULL, last_run_at TIMESTAMP, next_run_at TIMESTAMP, last_error TEXT, required_permission TEXT NOT NULL DEFAULT 'read_only', dry_run BOOLEAN NOT NULL DEFAULT true, approval_required BOOLEAN NOT NULL DEFAULT false, created_at TIMESTAMP NOT NULL DEFAULT now(), updated_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS life_automation_logs (id SERIAL PRIMARY KEY, automation_id TEXT NOT NULL REFERENCES life_automations(id) ON DELETE CASCADE, status TEXT NOT NULL, message TEXT NOT NULL, job_id TEXT, metadata JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS life_integrations (id TEXT PRIMARY KEY, provider TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'missing_config', mode TEXT NOT NULL DEFAULT 'read_only', last_sync_at TIMESTAMP, metadata JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT now(), updated_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS life_scores (id TEXT PRIMARY KEY, overall_score INTEGER NOT NULL, domains JSONB NOT NULL DEFAULT '{}'::jsonb, source TEXT NOT NULL, created_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS life_coach_sessions (id TEXT PRIMARY KEY, prompt_summary TEXT NOT NULL, priority_domain TEXT NOT NULL, confidence DOUBLE PRECISION NOT NULL, sources JSONB NOT NULL DEFAULT '[]'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS integration_signals (id TEXT PRIMARY KEY, integration TEXT NOT NULL, category TEXT NOT NULL, severity TEXT NOT NULL, summary TEXT NOT NULL, source_ref TEXT, metadata JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS memory_graph_v2_nodes (id TEXT PRIMARY KEY, type TEXT NOT NULL, label TEXT NOT NULL, summary TEXT, confidence DOUBLE PRECISION NOT NULL DEFAULT 1, source TEXT NOT NULL, metadata JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT now(), updated_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE TABLE IF NOT EXISTS memory_graph_v2_edges (id TEXT PRIMARY KEY, source_id TEXT NOT NULL REFERENCES memory_graph_v2_nodes(id) ON DELETE CASCADE, target_id TEXT NOT NULL REFERENCES memory_graph_v2_nodes(id) ON DELETE CASCADE, relation TEXT NOT NULL, confidence DOUBLE PRECISION NOT NULL DEFAULT 1, source TEXT NOT NULL, created_at TIMESTAMP NOT NULL DEFAULT now())`,
  `CREATE INDEX IF NOT EXISTS idx_life_events_created_at ON life_events(created_at DESC)`,
  `CREATE INDEX IF NOT EXISTS idx_life_events_category ON life_events(category, severity)`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_memory_graph_v2_nodes_label ON memory_graph_v2_nodes(label)`,
];
