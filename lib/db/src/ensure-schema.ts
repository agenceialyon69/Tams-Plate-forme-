...
function enumStatement(name: string, values: string[]): string {
...
async function waitForDatabase(attempts = 30, delayMs = 2000): Promise<void> {
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      console.error(`[db] pas encore joignable (tentative ${i}/${attempts}): ${detail}`);
...
export async function ensureSchema(): Promise<boolean> {
  try {
    ...
    const detail = err instanceof Error ? err.message : String(err);
    console.error(`[db] ensureSchema a échoué (le serveur continue): ${detail}`);
    return false;
  }
}
