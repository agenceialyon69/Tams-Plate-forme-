...
- **Dashboard premium dark theme** : refonte complète du tableau de bord avec
  ...
  hiérarchisées. Accent cyan/ambre pour signature visuelle premium.
- **Recovery & Export ZIP** : nouvelle page `/export` pour sauvegarde complète.
  Export JSON et ZIP avec database, manifest, README. Routes backend
  `/export/status`, `/export`, `/export/zip`. Aperçu des données par table.
  Procédure de restauration documentée.
- **Copilot avec upload de fichiers et Audit Red Team** :
  - Mode Audit Red Team pour analyse de risques, angles morts, conséquences
  - Support multi-fichiers (max 5, 25 Mo) : images, PDF, vidéos, audio, documents
  - Module `file-analyzer.ts` avec extraction métadonnées via FFprobe
  - Route POST `/copilot/audit` pour analyse structurée des pièces jointes
  - Réponses Red Team : risques, objections, recommandations
  - Interface premium avec drag & drop et preview des fichiers
...
## IN PROGRESS
- **Supabase database setup** : Created all tables via migrations with RLS policies enabled. 21 tables covering users, memory, tasks, decisions, captures, events, leads, copilot messages, audit logs, app events, etc. Full schema deployed with `001_initial_schema` and `002_rls_policies` migrations.
